...
**Revision:** 2
**Last modified:** 2026-05-22T17:15:00Z
...
> **R2 (v1.0.11):** `scripts/tmx-shell-init.sh` is now AUTO-GENERATED by
> `scripts/setup.sh` step 3a from `scripts/tmx-shell-init.sh.template`
> (sed-substitutes `__PROJECT__` + `__DATE__`). Before v1.0.11 the file
> was supposed to exist but no step generated it; the rc snippet's
> `[ -r ... ] && . ...` guard silently no-oped → no operator prompt
> fired. The template is tracked in git; the generated file is in
> `.gitignore` per §11.4.30. Removal happens via
> `scripts/uninstall.sh` or `bash scripts/setup.sh --uninstall`.
