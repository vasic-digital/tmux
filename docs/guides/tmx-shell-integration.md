...
### 7.2 Remove the state file (optional)

```bash
rm -rf ~/.tmx
```

This wipes per-session cwd memory. The next `tmx new` re-creates the
directory + an empty `state.json` automatically.
