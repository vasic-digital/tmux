...
| Pinned tmux | upstream release TAG `3.7b` (`e802909de06012a4df6209d55e86487c56223163`) — re-pinned by TMX-089 (`a6f3fc4`) from the untagged `next-3.8` (`40381bdc`) that `562cde7` had briefly adopted. `3.7b` is an ANCESTOR of `40381bdc` (verified — a clean rewind, not a divergence); its release tarball exists with measured sha256 `87f2e99e3b685973f2ca002ffd6ed7e51a5744f7009daae5a15670b6d532db96` and ships a pre-generated `configure` AND `cmd-parse.c`, which is what the root-free zig build path requires. Rebuilt binary reports `tmux 3.7b` (verified). Note: tag `3.7c` EXISTS upstream, so `3.7b` is NOT the latest stable and `README.md` no longer claims it is. |
| Version | **1.0.44** (versionCode 45, NOT yet released — no tag cut) — FOUR items this cycle: **TMX-086 + TMX-087** (the operator-reported install blocker) and, landing after that batch already reached `main`, **TMX-088** (the zig build path's `YACC` was an ABSOLUTE path, breaking BOTH `configure`'s `AC_CHECK_PROG` resolution AND `etc/ylwrap`'s output-filename contract — the latter a SILENT exit-0 failure; found by INDEPENDENT post-hoc review of `562cde7`, fixed in `7966edb`, new test 91 + mutation `M-ZIG-YACC-BARE-WORD`) and **TMX-089** (`562cde7`'s `next-3.8` pin withdrew the root-free build capability — test 71 C4 MEASURED the loss; re-pinned to release TAG `3.7b` in `a6f3fc4` on a second explicit operator decision the same day, tarball fast-path restored but now DERIVED from the submodule's `configure.ac` and refused without a matching pinned sha256). See §3.36 (cycle state) + `Fixed.md` §M (TMX-088/089). TMX-086 + TMX-087: TWO INDEPENDENT root causes behind the operator's single "cannot install / setup tmx" report, each one alone sufficient to block the install — (1) `install.sh` injected an HTTPS `insteadOf` rewrite BY DEFAULT, which git propagates into every recursive submodule clone via `GIT_CONFIG_PARAMETERS`, so it reached the one PRIVATE nested repo (`HelixDevelopment/helix_perf_cache`) and blocked forever on a GitHub `Username` prompt with no `GIT_TERMINAL_PROMPT=0` guard; (2) the constitution submodule's index carried a `.gitmodules`-unmapped duplicate `design-toolkit` gitlink, aborting every consumer's recursive clone with `Failed to recurse into submodule path 'constitution'`. Fixed by defaulting the clone URL to SSH + making the rewrite an explicit opt-in (`TMX_INSTALL_HTTPS_REWRITE=1`) + exporting the no-prompt / no-hang git env (`GIT_TERMINAL_PROMPT=0`, `GIT_ASKPASS=""`, `SSH_ASKPASS=""`, `GIT_SSH_COMMAND` with `accept-new`), and by constitution commit `b9096ac` (orphan gitlink removed, ff-pushed to all six upstreams, no force-push); constitution pointer bumped `6f24df9` → `b9096ac`. New test 90 (§11.4.115 RED/GREEN) + 3 paired mutations. See §3.35 + `Fixed.md` §L (TMX-086/087); §3.36 + `Fixed.md` §M (TMX-088/089). Previous: v1.0.43 (TMX-084 + TMX-085 — two false-result defects in the verification HARNESS itself, not the wrapper: `run_all.sh`'s verdict classifier misread an honest `SKIP-layer:` sub-check note (and any test's own internal `PASS=N FAIL=N SKIP=N` counters) as the test's overall verdict, and test 59's `RED_MODE` default made it fail 100% deterministically against a genuinely-fixed wrapper under a bare harness invocation — `Fixed.md` §K); v1.0.42 (TMX-083, systemd-oomd victim-avoidance on session scopes — `ManagedOOMPreference=avoid` added to every scope/slice creation site, closing a §11.4.108 layer-3 gap where `MemoryMax=infinity` was correct but ineffective against oomd's orthogonal PSI-pressure victim selection; renumbered from v1.0.41 to v1.0.42 per §11.4.113 merge-onto-latest-main after a concurrent `main` branch shipped a separate v1.0.41 install-verification-RED batch — see `Fixed.md` §J, TMX-083); v1.0.41 (install-verification-RED fix — 4 distinct root causes across tests 22/58/77/86/88, including a v1.0.40 documentation-layer bluff where the opencode.json fix was CLAIMED but never actually landed — self-caught and corrected); v1.0.40 (TMX-082, split-topology test reconciliation); v1.0.39 (TMX-079, no resource or lifetime limit by default — root cause of "sessions/processes killed despite ample host capacity"); v1.0.38 (TMX-076/077, scope-independent collision guard + meta-test mutations); v1.0.37 (host-adaptive CPUQuota default, interactive server-scope split TMX_SERVER_SPLIT=1, TMX_CLASSIFICATION export, OOM score adjustment). |
| Verification (this cycle, v1.0.44) | Controlled A/B for TMX-086 (same repo, same host, same minute, ONE variable): SSH as declared → `exit=0`, `6adcfb2 refs/heads/main`; with the installer's `insteadOf` rewrite → `exit=128`, `fatal: could not read Username for 'https://github.com'`. Anonymous-HTTPS census (all git config bypassed, no credential helper on host): 9 repos PUBLIC, `HelixDevelopment/helix_perf_cache` the sole PRIVATE one — the trigger. New test 90 (`90_install_ssh_only_no_credential_prompt.sh`) drives the real installer behind a recording `git` shim that captures the REAL argv + the INHERITED `GIT_TERMINAL_PROMPT`, so its assertions are runtime evidence, not a grep of the source (§11.4.226): GREEN `RED_MODE=0` → `PASS=4 FAIL=0 SKIP=0` exit `0`; RED `RED_MODE=1` (reconstructed pre-fix installer) → `PASS=1 FAIL=0 SKIP=0` exit `0`, defect reproduced. TMX-087: with the orphan gitlink removed, a recursive clone checked out `Containers` and `constitution` over SSH with NO credential prompt; constitution `b9096ac` ff-pushed (`6f24df9..b9096ac`) to all six upstreams, no force-push. A self-inflicted test-instrument FAIL-bluff (argv extractor keyed on `' clone'` with a leading space, matching only when the defect was present) was found and fixed at the instrument layer during authoring per §11.4.1. **TMX-088 measured:** `AC_CHECK_PROG` with `ac_word=/usr/bin/bison` → `found_yacc=no`, with `ac_word=bison` → `yes`, bogus-name negative control → `no`; real `tmux/etc/ylwrap` with `/usr/bin/bison` → `rc=0` and `cmd-parse.c` MISSING, with `bison -y` → `rc=0` and `cmd-parse.c` CREATED (both exit 0 — an exit-status assertion could never have caught it). New test 91 (`91_zig_yacc_contract.sh`, §11.4.115 RED/GREEN, `RED_MODE` default `0`) exercises BOTH real contracts without a full build and DERIVES the value under test from `build_native.sh` itself (§11.4.201(6) attributability); paired mutation `M-ZIG-YACC-BARE-WORD` verified to make C1+C2 FAIL. **TMX-089 measured:** under the `next-3.8` pin test 71 **C4** FAILED on its neutered bare host (`cmd-parse.c is not pre-generated and no yacc/bison found`) — the measured capability withdrawal; `3.7b` verified an ancestor of `40381bdc`; tarball sha256 `87f2e99e...d532db96` measured + pinned; rebuilt binary reports `tmux 3.7b`; tag `3.7c` confirmed to exist. **KNOWN-OPEN — explicitly NOT claimed (§11.4.6):** (1) **full-suite `run_all.sh` retest `UNCONFIRMED:`** — has NOT completed, outcome not known, still owed before the tag (§11.4.40); (2) **the full meta-test mutation sweep has NOT been run** this cycle — individual mutations verified, whole-sweep result not known; (3) **test 71 C4 has NOT been re-run on the `3.7b` artifact** — the root-free restoration is the RATIONALE for the re-pin, `UNCONFIRMED:` as a result; (4) **test 68 (`68_session_lifecycle.sh`) C6/C7 FAIL** — pre-existing, reproduces identically on the pre-change tree, TMX-080/TMX-081 family (`Issues.md` §H1/§H2), investigation in flight, no outcome recorded; (5) **`FAIL: T3: live WheelUpPane binding is not the copy-mode override`** (`17_scrollback_copy_mode.sh`) — real, currently failing, newly tracked as **TMX-090** (`Issues.md` §I1); under investigation, cause NOT established and none asserted; (6) **`.html`/`.pdf` twins STALE** (`pandoc`/`weasyprint` absent), remediation attempt in flight — an honest stale, never a faked transform (§11.4.106(E)). **Process note (§11.4.142/§11.4.113):** `562cde7` swept an agent's in-flight `build_native.sh` edit onto `main` before the mandatory independent review completed (`commit_all.sh` uses `git add -A`); remediated FIX-FORWARD in `7966edb`, no history rewrite, no force-push. Full detail: `Fixed.md` §L + §M. (Prior cycle, v1.0.43: investigated all 5 operator-reported failures (`27_state_persistence.sh 56_real_mouse_drag_copy.sh 57_reload_select_copy_paste.sh 59_oomd_preference_avoid.sh 87_server_scope_split.sh`) via `/superpowers:systematic-debugging`; 27/87 confirmed unchanged instances of the already-tracked TMX-080/TMX-081 (no action, still Queued); 56/57/59 root-caused to two harness-side classification bugs (TMX-084/085) — NEITHER was a defect in `scripts/tmx`/`scripts/tmx.template`. New regression test `89_classify_verdict_carrier.sh` (§11.4.115 RED/GREEN, 6 fixtures) + paired mutations `M-CLASSIFY-CARRIER`/`M-RED-DEFAULT-59`. Full detail: `Fixed.md` §K.) (Prior cycle, v1.0.40: `install.sh` clean `PASS=72 FAIL=0 SKIP=14`, real live-session split-topology verification via kernel `/proc/<pid>/cgroup`. Full detail: `Fixed.md` §I, TMX-082.) |
| Governance docs | `constitution/` submodule (HelixConstitution, pinned `b9096ac` as of v1.0.44 — bumped from `6f24df9` by the TMX-087 orphan-gitlink fix); `Containers/` submodule (pinned `18ed03d`); `tmux/` submodule pinned to release TAG `3.7b` (`e802909`) as of TMX-089 (`a6f3fc4`), re-pinned from the untagged `next-3.8` (`40381bdc`) — governance files and every version gate swept `next-3.8` → `3.7b`; project `Constitution.md` (Project Articles §101–§109 extends the submodule), `CLAUDE.md`, `AGENTS.md`, `QWEN.md`, `GEMINI.md` (all lockstep), `Issues.md`, `Fixed.md`, this document |
...
**Status:** IN PROGRESS. All FOUR fixes are landed on `main`; the release is **NOT tagged** and MUST NOT be tagged until the known-open list below is cleared. This entry is the single cycle-state record; §3.35 keeps the deep TMX-086/TMX-087 detail.

**Scope of this cycle — four items, two waves:**

| Item | What | Landed in |
|---|---|---|
| **TMX-086** | `install.sh` injected an HTTPS `insteadOf` rewrite BY DEFAULT; git propagated it into every recursive submodule clone via `GIT_CONFIG_PARAMETERS`, reaching the one PRIVATE nested repo (`HelixDevelopment/helix_perf_cache`) and blocking forever on a GitHub `Username` prompt with no `GIT_TERMINAL_PROMPT=0` guard. | `562cde7` |
| **TMX-087** | The constitution submodule's index carried a `.gitmodules`-unmapped duplicate `design-toolkit` gitlink, aborting every consumer's recursive clone. Fixed constitution-side in `b9096ac`, ff-pushed to all six upstreams, pointer bumped `6f24df9` → `b9096ac`. | `562cde7` (pointer) |
| **TMX-088** | The zig (root-free) build path computed `YACC` from `command -v bison` — an ABSOLUTE path — breaking BOTH contracts it feeds: `configure`'s `AC_CHECK_PROG` (`"$as_dir$ac_word"` → `/usr/bin//usr/bin/bison`, aborts `yacc not found` on a host that HAS bison) AND `etc/ylwrap`'s output-filename contract (plain `bison` emits `cmd-parse.tab.c`, ylwrap still EXITS 0 — a SILENT failure leaving make with no parser source). Fixed to a bare `bison -y`. Same commit: the autogen refusal message was UNREACHABLE under `set -euo pipefail` — fixed with `\|\| true`. | `7966edb` |
| **TMX-089** | `562cde7` had adopted the untagged `next-3.8` (`40381bdc`), which has NO release tarball — so the root-free path had to build from the submodule, needing autotools + bison. Test 71 **C4** MEASURED the withdrawal (`cmd-parse.c is not pre-generated and no yacc/bison found`), a §11.4.122 capability loss. Re-pinned to release TAG `3.7b` (`e802909`) on a SECOND explicit operator decision the same day. | `a6f3fc4` |

**How the second wave was found (§11.4.238 — the intended direction):** neither TMX-088 nor TMX-089 was operator-reported. TMX-088 came from an **independent post-hoc code review** of `562cde7`; TMX-089 came from a **test measuring a capability withdrawal**.

**Process note (§11.4.142 / §11.4.113) — recorded because it is the causal reason TMX-088 shipped broken:** `562cde7` swept an agent's IN-FLIGHT `scripts/build_native.sh` edit onto `main` before that edit had passed the mandatory independent review — `commit_all.sh` stages with `git add -A`, so unrelated in-flight work in the tree was carried along by a commit that was about something else. Remediated **FIX-FORWARD** in `7966edb` per §11.4.113: no history rewrite, no force-push at any point.

**Current pin + artifact (verified, not inferred):**

- Submodule gitlink → `e802909de06012a4df6209d55e86487c56223163`; `git -C tmux describe --tags` → `3.7b`.
- `3.7b` is an **ancestor** of `40381bdc` — a clean rewind, not a divergence.
- Release tarball sha256 `87f2e99e3b685973f2ca002ffd6ed7e51a5744f7009daae5a15670b6d532db96` (measured, pinned in `scripts/build_native.sh`); ships pre-generated `configure` AND `cmd-parse.c`.
- **Rebuilt binary reports `tmux 3.7b`** (read from the binary itself).
- `build_native.sh`'s tarball fast-path is restored but now DERIVES the version from the submodule's own `configure.ac` and REFUSES the download unless a sha256 is pinned for exactly that version — so a future re-pin to an untagged commit falls back to the submodule build rather than silently shipping a binary that does not match the pin.
- Governance files and every version gate swept `next-3.8` → `3.7b`. `README.md` no longer claims "latest stable" — tag `3.7c` exists (measured).

**KNOWN-OPEN RIGHT NOW — none of these is claimed fixed, and each blocks the tag or is tracked (§11.4.6 / §11.4.40):**

1. **Full-suite `run_all.sh` retest — `UNCONFIRMED:`.** Has NOT completed. Outcome not known, not claimed. Owed before any tag (§11.4.40).
2. **Full meta-test mutation sweep — NOT RUN this cycle.** The individual mutations (`M-INSTALL-*`, `M-ZIG-YACC-BARE-WORD`) were verified; the whole-sweep result is not known and is not claimed.
3. **Test 71 **C4** has NOT been re-run on the `3.7b` artifact.** The root-free restoration is the STATED RATIONALE for the re-pin — it is `UNCONFIRMED:` as a RESULT until C4 is re-run on a neutered host and captured. This is the missing GREEN half of TMX-089's RED→GREEN pair.
4. **Test 68 (`68_session_lifecycle.sh`) C6/C7 FAIL** — pre-existing; fails identically on the pre-change tree, so it is NOT caused by this cycle's work. Belongs to the already-tracked TMX-080 / TMX-081 family (`Issues.md` §H1/§H2). An investigation is IN FLIGHT; **no outcome is recorded here.**
5. **`FAIL: T3: live WheelUpPane binding is not the copy-mode override`** (`17_scrollback_copy_mode.sh`) — a real, currently-failing, previously-unrecorded defect, now tracked as **TMX-090** (`Issues.md` §I1). Under investigation; **the cause is NOT established and none is asserted.** First concrete step: capture the test's own `observed:` line (the live binding text), which the entry records as `PENDING_FORENSICS:`.
6. **`.html` / `.pdf` document twins are STALE** — `pandoc` / `weasyprint` are absent on this host; a remediation attempt is in flight. Honest stale per §11.4.106(E), never a faked transform.

**Immediate next actions (in order):**

1. Re-run test 71 **C4** against the `3.7b` artifact and capture the output — closes TMX-089's owed GREEN (item 3).
2. Run the full `run_all.sh` sweep + the full meta-test mutation sweep, capture both (items 1–2).
3. Capture the `observed:` binding text for TMX-090 and open the systematic-debugging arc (item 5).
4. Backfill the `Fixed.md` §L placeholder closure SHAs (see §3.35's "Placeholder owed"). §M already carries real SHAs (`7966edb`, `a6f3fc4`).
5. Only after 1–4: cut the tag (§11.4.40 full retest GREEN + §11.4.185 manual QA remain preconditions).

Full detail + captured evidence: `Fixed.md` §L (TMX-086, TMX-087) and §M (TMX-088, TMX-089); `CHANGELOG.md` v1.0.44; `Issues.md` §I1 (TMX-090).

### §3.35 — v1.0.44: SSH-only installability — credential prompt + orphan gitlink (TMX-086/TMX-087) → 2026-09-01

**Status:** DONE (both fixes landed + verified; release documentation authored this commit). Full-suite retest `UNCONFIRMED:` — see below.

**Trigger (verbatim operator report, 2026-09-01):** "We cannot install / setup tmx (tmux) on the System! Running our bash script gets stuck asking for GitHub credentials, which MUST NOTEVER haoppen! We shall rely only on ssh key (git protocol)!" Investigated via `/superpowers:systematic-debugging` per §11.4.102. Universal §11.4.238 applies — the defect was found by the OPERATOR attempting a real install, not by any automated gate this project ships; test 90 closes that coverage escape.

**Root cause (TWO INDEPENDENT defects behind ONE symptom — either alone blocks the install, so fixing only one would have left the operator still broken; §11.4.194 exhaustive-review discipline is what surfaced the second):**

- **TMX-086 — the credential prompt.** `scripts/install.sh` injected `-c url.https://github.com/.insteadOf=git@github.com:` BY DEFAULT. Git propagates `-c` flags to child processes via `GIT_CONFIG_PARAMETERS`, so the rewrite did NOT stay scoped to the top-level clone — it reached every RECURSIVE submodule clone. The constitution submodule nests `submodules/helix_perf_cache -> git@github.com:HelixDevelopment/helix_perf_cache.git`, the ONLY repo of the nine in the tree that is not anonymously readable; rewritten to HTTPS it cannot be fetched without credentials, so GitHub asks for a `Username`, and with nothing setting `GIT_TERMINAL_PROMPT=0` git BLOCKED on that prompt indefinitely. Under `curl | bash` the prompt is unanswerable. Anonymous-HTTPS census (all git config bypassed, no credential helper on host): PUBLIC = `vasic-digital/{tmux,Containers,token_optimizer,session_orchestrator,continuum,anti_bluff,design-toolkit,docs_chain}` + `HelixDevelopment/HelixConstitution`; PRIVATE = `HelixDevelopment/helix_perf_cache` (the trigger).
- **TMX-087 — the orphan gitlink.** The constitution submodule's index carried TWO gitlinks at the identical SHA `efd2c3fb2f880aaf2baf7f4819ce28a1ce3609cb`: a bare `design-toolkit` with NO `.gitmodules` mapping, and the correctly-mapped `submodules/design-toolkit`. Git refuses to recurse into an unmapped gitlink, so every consumer's recursive clone aborted with `fatal: No url found for submodule path 'constitution/design-toolkit' in .gitmodules` / `fatal: Failed to recurse into submodule path 'constitution'`. Introduced when the submodule was relocated under `submodules/` (constitution commit `e499ffb`) without removing the old index entry.

**Fix:**

- **TMX-086** (all in `scripts/install.sh`): `DEFAULT_REPO_URL` changed to `git@github.com:vasic-digital/tmux.git`; the `insteadOf` rewrite is OFF by default and now an explicit opt-in via `TMX_INSTALL_HTTPS_REWRITE=1` / `--https-rewrite`, with the legacy `TMX_INSTALL_NO_HTTPS_REWRITE=1` / `--no-https-rewrite` still honoured as an opt-OUT that WINS (§11.4.122 — the capability is preserved, only its default changed); exported `GIT_TERMINAL_PROMPT=0`, `GIT_ASKPASS=""`, `SSH_ASKPASS=""` so a missing credential fails fast instead of hanging; exported `GIT_SSH_COMMAND="${GIT_SSH_COMMAND:-ssh -o StrictHostKeyChecking=accept-new -o ConnectTimeout=20}"` so a first-contact github host key cannot hang either. Honest boundary (§11.4.6): `accept-new` still REFUSES a CHANGED key, an ssh key passphrase prompt is deliberately still allowed, and an operator-set `GIT_SSH_COMMAND` is never overridden.
- **TMX-087**: constitution commit `b9096ac` removes ONLY the orphan gitlink — `.gitmodules` untouched, `submodules/design-toolkit` keeps the same pinned SHA, so no capability is dropped. Pushed fast-forward (`6f24df9..b9096ac`) to all SIX constitution upstreams (`github`, `gitlab`, `gitflic`, `gitverse`, `vasic_digital_github`, `vasic_digital_gitlab`), no force-push (§11.4.113). This repo's constitution pointer is bumped `6f24df9` → `b9096ac`.

**Tests:** new `scripts/tests/90_install_ssh_only_no_credential_prompt.sh` (§11.4.115 RED/GREEN, `RED_MODE` default `0` per the TMX-085 convention). Method: a recording `git` shim placed first on `PATH` captures the REAL argv and the INHERITED `GIT_TERMINAL_PROMPT`, so the assertions are RUNTIME evidence, not a grep of the installer source (§11.4.226). Sub-checks: **A** default clone URL uses the SSH form; **B** no `insteadOf` rewrite in the real git argv by default; **C** git inherited `GIT_TERMINAL_PROMPT=0`; **D** `TMX_INSTALL_HTTPS_REWRITE=1` still enables the rewrite (capability preserved). `RED_MODE=1` reconstructs a pre-fix copy of `install.sh` and asserts the rewrite IS injected — proving the test is not blind. Three paired mutations in `meta_test_false_positive_proof.sh`: `M-INSTALL-SSH-DEFAULT` (expect `FAIL: A`), `M-INSTALL-HTTPS-REWRITE-DEFAULT` (expect `FAIL: B`), `M-INSTALL-NO-PROMPT-GUARD` (expect `FAIL: C`).

**Measured verification:**

- Controlled A/B (same repo, same host, same minute, ONE variable): SSH as declared → `exit=0`, `6adcfb2 refs/heads/main`; with the installer's rewrite → `exit=128`, `fatal: could not read Username for 'https://github.com'`.
- Test 90 GREEN (`RED_MODE=0`, default) → `PASS=4 FAIL=0 SKIP=0`, exit `0`. Test 90 RED (`RED_MODE=1`) → `PASS=1 FAIL=0 SKIP=0`, exit `0`, defect reproduced on the pre-fix artifact.
- Live clone observation: with the orphan gitlink removed, `Containers` and `constitution` checked out over SSH with NO credential prompt.
- **`UNCONFIRMED:` full-suite `run_all.sh` retest** — still in flight when this entry was authored; its outcome is not known here and is NOT claimed (§11.4.6 / §11.4.40 remains owed before the tag is cut).

**Test-instrument bug found and fixed during authoring (§11.4.1 FAIL-bluff / §11.4.201(7)(c)):** test 90's first argv extractor keyed on `' clone'` WITH A LEADING SPACE, which matched ONLY when the defect was present (the pre-fix installer inserts `-c ...` before `clone`), producing a false FAIL on correct code. Fixed at the instrument layer to a word-boundary match and documented inline in the test.

**Placeholder owed (§11.4.6):** `Fixed.md` §L carries `(this commit — MUST be backfilled with the real SHA on landing)` for both items' tmux-side closure commit — the same never-backfilled-placeholder class already corrected for TMX-079 / TMX-082 / TMX-083. Backfill on landing.

Full detail + captured evidence: `Fixed.md` §L (TMX-086, TMX-087); `CHANGELOG.md` v1.0.44.
