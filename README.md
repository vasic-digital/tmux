...
| Tracker + review gates (§11.4.19/§11.4.134) | [`docs/scripts/tracker_structural_integrity.md`](docs/scripts/tracker_structural_integrity.md) — Issues/Fixed section-survival gate; [`docs/scripts/review_round_record.md`](docs/scripts/review_round_record.md) — append-only review-round ledger; [`docs/scripts/heading_grammar_gate.md`](docs/scripts/heading_grammar_gate.md) — tracker heading-grammar gate (every `###` heading carries a block code, mirroring `parser.go`'s `headingRE`). All three run from `verify.sh` as `CM-TRACKER-STRUCTURAL-INTEGRITY` / `CM-REVIEW-ROUND-RECORD-WIRED` / `CM-HEADING-GRAMMAR` |
