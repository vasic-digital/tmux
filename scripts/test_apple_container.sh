#!/usr/bin/env bash
# test_apple_container.sh — build + test the LINUX tmux 3.7b inside a real
...
#   is the project's `tmux/` submodule (pin = release tag 3.7b, `configure` present).
#
# ── Cross-references ──────────────────────────────────────────────────
...
build_tmux() {
    log "building Linux tmux 3.7b inside container (timeout ${BUILD_TIMEOUT}s)…"
