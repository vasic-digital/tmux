        ...
        if [ "$_gone" -ne 1 ]; then
            ...
            if ! _wrap_in_pane "drv_${C6NAME}_re" new -s "$C6NAME"; then
                ...
                if pth_wait_attached "$TMUX_BIN" "$C6SOCK" "$C6NAME" "1" 12; then
                    ...
                    if pth_wait_text "drv_${C6NAME}_re" "C6PWD_$_iter" 10; then _wt=0; else _wt=1; fi
                    if pth_capture "drv_${C6NAME}_re" | grep -qF "$PROJ"; then _g1=0; else _g1=1; fi
                    sleep 3
                    if pth_capture "drv_${C6NAME}_re" | grep -qF "$PROJ"; then _g2=0; else _g2=1; fi
                    if pth_wait_text "drv_${C6NAME}_re" "$PROJ" 10; then _wp=0; else _wp=1; fi
                    echo "DISCRIM C6 iter=$_iter: wait_marker=$_wt grep_now=$_g1 grep_after_3s_settle=$_g2 wait_for_PATH=$_wp"
                    if [ "$_wt" -eq 0 ] && [ "$_g1" -eq 0 ]; then
    ...
    if [ "$_recall_after" = "$PROJ" ]; then
        ...
        if ! _wrap_in_pane "drv_${NAME}_d" new -s "$NAME"; then
            ...
            if pth_wait_attached "$TMUX_BIN" "$SOCK" "$NAME" "1" 12; then
                ...
                if pth_wait_text "drv_${NAME}_d" "C7PWD_$_iter" 10; then _wt=0; else _wt=1; fi
                if pth_capture "drv_${NAME}_d" | grep -qF "$HOME_DIR"; then _g1=0; else _g1=1; fi
                sleep 3
                if pth_capture "drv_${NAME}_d" | grep -qF "$HOME_DIR"; then _g2=0; else _g2=1; fi
                if pth_wait_text "drv_${NAME}_d" "$HOME_DIR" 10; then _wp=0; else _wp=1; fi
                echo "DISCRIM C7 iter=$_iter: wait_marker=$_wt grep_now=$_g1 grep_after_3s_settle=$_g2 wait_for_PATH=$_wp"
                if [ "$_wt" -eq 0 ] && [ "$_g1" -eq 0 ]; then
