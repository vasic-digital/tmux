#!/usr/bin/env bash
# setup_tmux.sh — ONE-COMMAND orchestrator for the vasic-digital optimized tmux
# build & install.
#
...
#
# Usage: bash scripts/setup_tmux.sh             — full pipeline
#        bash scripts/setup_tmux.sh --uninstall — remove .bashrc snippet + ~/.tmux.conf
#        bash scripts/setup_tmux.sh --rebuild   — force re-run of containerized build
#        bash scripts/setup_tmux.sh --build-only — stop after step 2 (no install)
#        bash scripts/setup_tmux.sh --verify-only — stop after step 4 (no install)
...
if [ "$MODE" = "uninstall" ]; then
    ...
    if [ -f ~/.tmux.conf ] && grep -q 'vasic-digital optimized tmux configuration' ~/.tmux.conf; then
        rm -f ~/.tmux.conf
        echo "  ✓ removed ~/.tmux.conf (was generated by setup_tmux.sh)"
    fi
    rm -f scripts/tmx
...
echo "[setup_tmux] step 1 — host capability check"
ENGINE=""
...
if [ -z "$JEMALLOC" ]; then
    ...
    echo "    Recommend: sudo bash scripts/install_tmux_deps.sh"
else
...
if [ "$MODE" = "build-only" ]; then
    echo "[setup_tmux] --build-only: stopping after step 2"
    exit 0
fi

# Step 3 — generate wrapper
echo ""
echo "[setup_tmux] step 3 — generating tmx wrapper"
...
echo ""
echo "[setup_tmux] step 3b — building oom_set helper (no install)"
...
echo ""
echo "[setup_tmux] ✓ verification GREEN — safe to proceed with installation"

if [ "$MODE" = "verify-only" ]; then
    echo "[setup_tmux] --verify-only: stopping after step 4. PATH NOT exported."
    exit 0
fi

# Step 5 — install ~/.tmux.conf + .bashrc snippet (gated)
echo ""
echo "[setup_tmux] step 5 — installing user config"
...
echo "  Then 'tmux' will use the verified ATMOSphere build."
echo "════════════════════════════════════════════════════════════════"
